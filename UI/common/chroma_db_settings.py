...
def get_unique_sources_df_old(chroma_settings):
...
def get_unique_sources_df(chroma_settings):
    ...
    collection_data = chroma_settings.get_collection('vectordb').get(include=["metadatas"])

    # Create DataFrame with proper structure
    if collection_data and "metadatas" in collection_data and collection_data["metadatas"]:
        # Extract sources from metadatas
        sources = [meta.get('source', None) for meta in collection_data["metadatas"] if meta]
        sources = [s for s in sources if s]  # Filter out None values

        # Get just the filenames (not full paths)
        file_names = [source.split('/')[-1] for source in sources]

        # Create DataFrame with unique sources
        unique_sources = list(set(file_names))
        df = pd.DataFrame({"source": unique_sources})

        # Rename the column to 'Archivo' as in the old function
        df.rename(columns={'source': 'Archivo'}, inplace=True)

        return df
    else:
        # Return empty DataFrame with the correct column name
        return pd.DataFrame({"Archivo": []})